...
from flcore.servers.server_avg_mask_adaptive import FedAvgMaskAdaptive
from flcore.servers.server_avg_mask import FedAvgMask
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "MLR": # convex
            ...
            elif "CIFAR10" in args.dataset or "CIFAR10_noniid" in args.dataset:
        ...
        elif args.algorithm == "FedAvgMaskAdaptive":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedAvgMaskAdaptive(args, i)

        elif args.algorithm == "FedAvgMask":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedAvgMask(args, i)
    ...
    # Global average
    average_data(dataset=args.dataset, algorithm=args.algorithm, goal=args.goal, times=args.times, test_name=args.save_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', "--model", type=str, default="MobileNet")#ResNet18
    ...
    parser.add_argument('-ls', "--local_epochs", type=int, default=3,
                        help="Multiple update steps in one local epoch.")   # 1, 改成3了
    parser.add_argument('-algo', "--algorithm", type=str, default="FedAvgMaskAdaptive") # FedAvg,改成我们的FedAvgMask了
    ...
    parser.add_argument('-bnpc', "--batch_num_per_client", type=int, default=2) # 2，改成8了
    ...
    # FedAvgMaskAdaptive
    parser.add_argument('-min_rate', "--min_compress_rate", type=float, default=0.8)
    parser.add_argument('-max_rate', "--max_compress_rate", type=float, default=0.99)
    parser.add_argument('-cbits', "--compress_bits", type=int, default=2)
    parser.add_argument('-adaptive_rate', "--adaptive_rate", type=float, default=5)
    parser.add_argument('-save_name', "--save_name", type=str)

    parser.add_argument('-model_str', "--model_str", type=str,default="MobileNet")


    # conda activate fzy_pfllib
